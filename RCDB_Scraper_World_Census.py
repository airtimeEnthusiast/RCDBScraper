...
#####################################################
 ...
 def parse_extant_coasters():
  ...
  ####################################################
  # Parse through  all the pages of coasters  (Census)   
  ####################################################
 def parse_all_coasters():
  columns = ["Name","Park","City","State","Country","Status","Status Date","Material","Seating","Thrill","Make","Model","Length","Height","Drop","Speed","Inversions","VerticalAngle","Duration","G-Force"]
  link = "https://rcdb.com/census.htm"                #List of existing coasters in the US (page 1)

  # Get request from the world census page
  #response = requests.get(link)                             #Get Response
  #soup = bs(response.text,"html.parser")             #Create Responser
  #CountryName = 'Unitied States'                            #The Country of Census choice
  #soup = soup.body.find('form',{'class' : 'sanitize'}).find('div', { 'class' : 'stdtbl crx'})
  #references = soup.table.tbody.findAll('tr')
  #References in the table
  #for reference in references:
   #ref_link = reference.find('td', { 'data-sort' : 'North America'})
   #print(ref_link)
   #if ref_link != None:
    #ref_links = ref_link.findAll('td')
    #for rl in ref_links:
     #country = rl.find('North America')
     #if country != None:

  # The coaster census page request
  #response = requests.get(ref_link)
  #soup = bs(response.text "html.parser")
  
  
  #print(soup)
  #num_extants = soup.body.find('form',{'class' : 'sanitize'}).find('div', { 'class' : 'stdtbl crx'}) #Get total number of extisting coasters in the country
  #pages = Parser.get_extant_pages(link)
  #frames = [None] * len(pages)
  #print("Parsing" + str(len(frames)))
  #for i in range(len(pages)):
   #Parse 
   #frames[i] = Parser.parse_extant_coasters_page(pages[i])
   #sleeptime = random.randint(80,520)
   #Sleep Time
   #print("Sleeping for" + sleeptime)
   #time.sleep(random.randint(80,520))
  #data_frame = pd.concat(frames)
  #data_frame.set_axis(columns, axis=1)
  #data_frame.to_csv("US_Coaster_Stats_2021.csv")   
...
######
def tester():
 response = requests.get("https://rcdb.com/r.htm?ot=2&ol=59&ex")
 soup = bs(response.text,"html.parser")     #Create Responser
 coasters_on_page = soup.body.section.find("div", {"class" : "stdtbl rer"}).find("table").find_all("tr") #Find table references of coasters on the next page
 print(str(len(coasters_on_page)) + " coasters on page ")
 #num_extants = soup.body.find("table").select_one("tr:nth-(1)").td.a.get_text()    #Get total number of extisting coasters in the country
 coaster_data_array = [[] for r in range((len(coasters_on_page) - 1))]                                       #Store data in an array
 ...
 for i in range(2,len(coasters_on_page)):
  ...
  _.to_csv("US_Coaster_Stats_2021.csv")
 return _
